...
adminRouter.route("/validateAdmin")
    .get([],
            function (req, res) {
                let adminDetail = _.pick(req.query, ['emailAddress', 'password']);
                adminFacade.validateAdmin(adminDetail)
                    .then(function (result) {
                        resHndlr.sendSuccessWithMsg(res, result);
                    })
                    .catch(function (err) {
...
adminRouter.route("/addTeacher")
    .post([], function (req, res) {
        let teacherDetail = _.pick(req.body, ["teacherSkypeID", "teacherEmailID","teacherCountry","teacherContactNumber","teacherName"]);
